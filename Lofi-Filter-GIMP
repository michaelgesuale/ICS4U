...
def lo_fi(image, to_draw):
    pdb.gimp_image_undo_group_start(image)
    s_curve = (0, 0, 96, 64, 128, 128, 160, 192, 255, 255)
    inverted_s_curve = (0, 0, 64, 96, 128, 128, 192, 160, 255, 255)
    num_points = 10
    pdb.gimp_curves_spline(to_draw, HISTOGRAM_RED, num_points, s_curve)
    pdb.gimp_curves_spline(to_draw, HISTOGRAM_GREEN, num_points, s_curve)
    pdb.gimp_curves_spline(to_draw, HISTOGRAM_BLUE, num_points,
                           inverted_s_curve)
...
register(
    "python-fu-lofi",
    "Lo Fi filter",
    "Recreates Instagram's Lo-Fi filter",
    "Michael Gesuale", "Michael Gesuale", "2016",
    "Lomo",
    "RGB",
    [
        (PF_IMAGE, "image", "takes current image", None),
        (PF_TO_DRAW, "to draw", "Input layer", None)
    ],
    [],
    lomo, menu="/Filters")
